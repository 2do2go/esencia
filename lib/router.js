...
(function (root, factory) {
	...
	/*Router.processController = function(controller, stage, callback) {
		...
		if (parentName) {
			...
			this.processController(parent, parentStage, function() {
				...
				if (stage != 'renderOnly') {
					controller.view = new (controller.View)(
						_(controller)
							.chain()
							.pick('models', 'collections', 'urlParams', 'container')
							.defaults(controller.viewOptions)
							.value()
					);
				}
	...
	Router.processController = function(controller, stage, callback) {
		...
		if (parentName) {
			var parentStage = 'createAndRender';
			...
			if (parent.view && parent.view.isAttached()) parentStage = 'render';
			...
			this.processController(parent, parentStage, function() {
				...
				self._runStage(controller, stage, callback)
			});
		} else {
			self._runStage(controller, stage, callback)
		}
	...
	Router._runStage = function(controller, stage, callback) {
		if (stage != 'render') {
			controller.view = new (controller.View)(
				_(controller)
					.chain()
		...
		if (controller.view.needWaitForInit) {
			this.listenTo(controller.view, 'viewInitialize' + controller.view.cid, function() {
				controller.view.render();
				callback();
			})
		} else {
